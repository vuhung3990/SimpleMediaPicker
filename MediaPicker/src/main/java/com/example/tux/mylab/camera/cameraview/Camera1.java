...
//            final boolean needsToStopPreview = mShowingPreview && Build.VERSION.SDK_INT < 14;
  ...
  /**
   * Calculate display orientation https://developer.android.com/reference/android/hardware/Camera.html#setDisplayOrientation(int)
   * <p> This calculation is used for orienting the preview <p> Note: This is not the same
   * calculation as the camera rotation
   *
  ...
  /**
   * Calculate camera rotation <p> This calculation is applied to the output JPEG either via Exif
   * Orientation tag or by actually transforming the bitmap. (Determined by vendor camera API
   * implementation) <p> Note: This is not the same calculation as the display orientation
   *
   * @param screenOrientationDegrees Screen orientation in degrees
  ...
  private static class MediaPrepareTask extends AsyncTask<MediaRecorder, Void, Boolean> {
    ...
    protected void onPostExecute(Boolean result) {
      if (result) {
        mediaPrepareListener.onSuccess();
      } else {
        mediaPrepareListener.onError();
      }
